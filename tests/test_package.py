...
class PyRsyncTests(unittest.TestCase):
    ...
    def get_delta(self, file):
        ...
        hashes = pyrsync2.blockchecksums(file_to, blocksize=self.TEST_BLOCK_SIZE)

        delta = pyrsync2.rsyncdelta(file_from, hashes, blocksize=self.TEST_BLOCK_SIZE)
        return list(delta)

    def test_blockchecksums(self):
        with BytesIO(self.TEST_FILE) as file1:
            hashes = pyrsync2.blockchecksums(file1, blocksize=self.TEST_BLOCK_SIZE)
        ...
        with BytesIO(self.TEST_FILE) as file_to:
            ...
            with BytesIO(self.TEST_FILE) as file_from:
                delta = pyrsync2.rsyncdelta(
                    file_from, hashes, blocksize=self.TEST_BLOCK_SIZE
                )
            ...
            with BytesIO(self.TEST_FILE) as file_to:
                ...
                for block, data in enumerate(delta):
                    if block in changed_blocks:
                        self.assertEqual(self.get_block(changed_file_data, block), data)
                    else:
    ...
    def test_patchstream_with_additions(self):
        ...
        out_file = BytesIO()
        pyrsync2.patchstream(old_file, out_file, delta, blocksize=self.TEST_BLOCK_SIZE)
